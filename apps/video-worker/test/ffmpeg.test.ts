...
test("FFmpeg receives a safe argument array and configured executable/timeout", async () => {
  ...
  const execute = (async (file: string, received: string[], options: { timeout: number }) => {
    ...
    assert.deepEqual(received.slice(0, 6), [
      "-nostdin",
      "-hide_banner",
      "-loglevel",
      "error",
    ...
    for (const value of [
      ...
      "+faststart",
      "file,pipe",
      "/tmp/input with spaces",
    ])
      assert.ok(received.includes(value));
...
test("corrupt media is permanent, missing binary and timeouts remain retriable", async () => {
  for (const [error, permanent] of [
    [{ stderr: "Invalid data found when processing input" }, true],
    [{ code: "ENOENT" }, false],
    [{ timedOut: true }, false],
  ] as const) {
    const execute = (async () => {
    ...
    await assert.rejects(
      runFfmpeg([], config, execute),
      (result) => result instanceof NonRetriableError === permanent,
    );
  }
});
test("temporary directories are unique and cleaned after success and failure", async () => {
  ...
  await withMediaDirectory(async (directory) => {
    first = directory;
    await withMediaDirectory(async (other) => {
      second = other;
    });
  });
