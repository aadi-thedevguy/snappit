...
export async function withMediaDirectory<T>(work: (directory: string) => Promise<T>, parent = tmpdir()) {
...
export async function runFfmpeg(args: string[], config: MediaConfig, execute: typeof execa = execa) {
  try {
    await execute(config.FFMPEG_PATH, ["-nostdin", "-hide_banner", "-loglevel", "error", "-y", ...args], {
    ...
    if (/Invalid data found when processing input|moov atom not found|does not contain any stream|matches no streams/i.test(failure.stderr ?? "")) {
    ...
    throw new Error(failure.timedOut ? "FFmpeg exceeded the processing time limit" : "FFmpeg failed: Unable to process media", { cause: error });
  }
}
