...
import { runFfmpeg as execute, transcodeToMp4 as transcode, withMediaDirectory } from "./ffmpeg.js";
const config = {
  FFMPEG_PATH: process.env.FFMPEG_PATH || "/usr/bin/ffmpeg",
  FFMPEG_TIMEOUT_MS: 30000,
};
const runFfmpeg = (args, timeout = config.FFMPEG_TIMEOUT_MS) =>
  execute(args, { ...config, FFMPEG_TIMEOUT_MS: timeout });
const transcodeToMp4 = (input, output) => transcode(input, output, config);
...
test("media directories are unique and removed after success and failure", async () => {
  ...
  await withMediaDirectory(async (directory) => {
    first = directory;
    await withMediaDirectory(async (nested) => {
      second = nested;
    });
...
for (const audio of [false, true]) {
  test(`transcodes odd-sized uploaded video, audio=${audio}, with faststart`, async () => {
    await withMediaDirectory(async (directory) => {
      ...
      assert.ok(
        mp4.indexOf("moov") < mp4.indexOf("mdat"),
        "metadata must precede media for seeking",
      );
...
test("FFmpeg timeouts terminate the process", async () => {
  await assert.rejects(
    runFfmpeg(["-re", "-f", "lavfi", "-i", "testsrc=size=16x16:rate=1", "-f", "null", "-"], 100),
    /time limit/,
  );
});
