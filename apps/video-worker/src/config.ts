...
const schema = z.object({
  ...
  if (value.NODE_ENV === "production" && value.INNGEST_DEV === "1") {
    ctx.addIssue({ code: "custom", path: ["INNGEST_DEV"], message: "Development mode is forbidden in production" });
  }
  if (value.INNGEST_DEV !== "1") {
    for (const field of ["INNGEST_SIGNING_KEY", "INNGEST_EVENT_KEY"] as const) {
      if (!value[field]) ctx.addIssue({ code: "custom", path: [field], message: "Required for cloud mode" });
    }
  }
  if (Boolean(value.AWS_ACCESS_KEY_ID) !== Boolean(value.AWS_SECRET_ACCESS_KEY)) {
    ctx.addIssue({ code: "custom", path: ["AWS_ACCESS_KEY_ID"], message: "Supply both AWS credential fields or use the default credential provider" });
  }
  if (value.NODE_ENV === "production" && value.DATABASE_SSL === "disable") {
    ctx.addIssue({ code: "custom", path: ["DATABASE_SSL"], message: "TLS is required in production" });
  }
});
...
export function readConfig(env: NodeJS.ProcessEnv) {
  ...
  if (!result.success) {
    ...
    throw new Error(`Invalid worker configuration: ${result.error.issues.map(i => i.path.join(".")).join(", ")}`);
  }
  return result.data;
}
