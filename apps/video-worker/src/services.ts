...
export function createRenderer(
  config: WorkerConfig,
  storage: ReturnType<typeof createVideoStorage>,
  encode = transcodeToMp4,
) {
  return async (rawKey: string, processedKey: string) => {
    return withMediaDirectory(async (directory) => {
      ...
      const object = await storage.client.send(
        new GetObjectCommand({ Bucket: storage.bucket, Key: getVideoObjectKey(rawKey) }),
        { abortSignal: signal },
      );
      ...
      const limit = new Transform({
        transform(chunk, _encoding, callback) {
          ...
          callback(
            received > MAX_VIDEO_SIZE
              ? new NonRetriableError("Uploaded video exceeds size limit")
              : null,
            chunk,
          );
        },
      ...
      const upload = new Upload({
        ...
        params: {
          ...
          Key: getVideoObjectKey(processedKey),
