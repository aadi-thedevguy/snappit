...
export interface ProcessingServices {
  ...
  render(
    rawKey: string,
    processedKey: string,
  ): Promise<{ storageKey: string; renderTimeMs: number }>;
}
  ...
  return {
    async process({
      ...
      const video = await step.run("mark-processing", async () => {
        ...
        if (claimed.processingStatus === "uploading")
    ...
    async failure({
      ...
      await step.run("mark-video-processing-failed", () =>
        repository.fail(videoId, event.data.run_id),
      );
    },
  };
}
